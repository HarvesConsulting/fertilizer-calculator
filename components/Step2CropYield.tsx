    ...
    <div className="flex flex-col">
        ...
        <div className="relative">
            <input
                ...
                step={step}
                min="0"
                className="w-full pl-3 pr-16 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                required
            />
            <span className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 dark:text-gray-400">{unit}</span>
        </div>
    </div>
);
...
const SelectField: React.FC<{ label: string; name: string; value: string; onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void; options: {value: string; label:string}[]; placeholder: string; disabled?: boolean; }> =
    ...
    <div className="flex flex-col">
        ...
        <select
            ...
            onChange={onChange}
            disabled={disabled}
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:disabled:bg-gray-600"
        >
...
export const Step2CropYield: React.FC<Step2Props> = ({ onBack, onCalculate, data }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8">
            <h2 className="text-2xl font-semibold text-gray-800 dark:text-gray-100 border-b dark:border-gray-700 pb-4 mb-6">Крок 2: Культура та врожайність</h2>
            ...
            <div className="flex flex-col-reverse sm:flex-row justify-between items-center pt-4 gap-4">
                 <button
                    type="button"
                    onClick={onBack}
                    className="w-full sm:w-auto bg-gray-300 text-gray-800 font-bold py-3 px-10 rounded-lg hover:bg-gray-400 transition duration-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500"
                >
                    Назад
                </button>
                <button
